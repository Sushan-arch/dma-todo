...
public class AddTodoActivity extends AppCompatActivity {
    ...
    public void onSaveButtonClicked() {
        ...
        String time = (new StringBuilder().append(hour).append(":").append(min).append(" ").append(format)).toString();

        Date date = new Date();

        // TODO (4) Make todo final so it is visible inside the run method
        final Todo todo = new Todo(title, description, priority, time, date);
